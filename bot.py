...
async def info(event):
  await event.reply("**Merhaba Benim Ve Sahibim Hakkında Bilgi\n\nPython: 3.8.2\nKütüphanem: Telethon\n\nSahibim: @hi Gruplarınızda Üyeleri Etiketlemek için Yaratılmışım**",
                    buttons=(
                      ...
                      [
                       Button.url('Sahibim 🖥️', 'https://t.me/MajesteSahip')
                      ],
                    ),
                    link_preview=False
                   )
  ...
  if event.is_private:
    async for usr in client.iter_participants(event.chat_id):
     ...
     return await event.reply(f"**Merhaba \nGrubunuzdakı Üyeleri Etiketleye Bilirim\nKomutlar için Komutlar Düğmesine Tıklaya Bilirsiz**", buttons=(
                      ...
                      [
                       Button.url('Sahibim', 'https://t.me/MajesteSahip')
                      ],
                    ),
                    link_preview=False)
...
async def handler(event):
    async for usr in client.iter_participants(event.chat_id):
     ad = f"[{usr.first_name}](tg://user?id={usr.id}) "
     await event.edit(f"**Merhaba Ben @Majeste_TaggerBot\nGrubunuzdakı Üyeleri Etiketleye Bilirim\nKomutlar için Komutlar Düğmesine Tıklaya Bilirsiz**", buttons=(
                      [
                       Button.inline("Komutlar", data="komutlar")
                      ],
                      [
...
app.run()
print(">> Bot çalışıyor @MajesteSahip Tarafından Kuruldu<<")
